function [pdf, Coords]= compute_pdfGUI(Data,N,bin_scheme,Range,method,zeffect)
...
%5/8/18: updated to fix bugs with local binning method and fixed binning
%5/8/18: added zeffect option - same dimensions as 1 x dim, 1 or 0 (only
%applied if KDE method is also chosen
...
if strcmp(method,'KDE')==1
    ...
    for i =1:dim
        if zeffect(i)==1
            ...
            vect(vectdiff<1)=zval(i);
            Data(:,i)=vect;
            kx(i) = sum(Data(:,i)~=zval(i))./nTup; %proportions of non-zero values
        else
            zval(i) = 10^12; %set to high value (zero-effect is effectively not used)
            kx(i)=1;
        end
    end
    ...
    for n =1:nTup %look at each data point separately
        ...
        elseif dim ==2
           ...
           if dat(1)==zval(1) && dat(2)~=zval(2) %pyo: zero effect for x
                ...
                pdfy = pdfy + pdfyadd;
           elseif dat(1)~=zval(1) && dat(2)==zval(2) %pxo: zero effect for y
...
if strcmp(method,'fixed')==1 %%%%%%%%%%% Fixed Bin Method  %%%%%%%%%%%%%%%%%
