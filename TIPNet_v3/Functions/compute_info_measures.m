function [info]= compute_info_measures(pdf)
...
if dim ==3  %compute: H(X), H(Y), H(Z),
    ...
    sumpdf = sum(sum(sum(pdf)));
    sumpdf = round(sumpdf.*100)./100;
    
%     if sumpdf~=1
%         sprintf('sum of pdf = %5.4f\n',sum(sum(sum(pdf))))
%     end
...
%                         
%                         if T_add>100000
%                             sprintf('very high T, m_ijk = %4.3f\n',m_ijk)
%                             sprintf('m_ij = %8.7f, m_jk = %8.7f, m_j =%8.7f\n',m_ij(i,j),m_jk(j,k),m_j(j)) 
%                         end
                    end
                end
...
if R<0
    ...
    if I_tot>1000
        sprintf('ahhh infinity %10.3f, %10.3f, %10.3f\n', T, I_x1y, I_x2y)
