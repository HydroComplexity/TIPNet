...
% Last Modified by GUIDE v2.5 08-May-2018 15:18:42
...
if mi.DataPrep.Z_effect(1)==1
set(handles.check_zeroeffect,'Value',1)
end
...
set(handles.enter_segsteps,'String',num2str(mi.segsteps));
set(handles.check_zeroeffect,'Value',0);
...
% --- Executes on button press in check_zeroeffect.
function check_zeroeffect_Callback(hObject, eventdata, handles)
% hObject    handle to check_zeroeffect (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

check_z = get(handles.check_zeroeffect,'Value');
...
mi.DataPrep.Z_effect(Selected)=check_z;

if check_z ==0
    mi.DataPrep.Z_effect(1)=0;
end

%if all nodes selected
if Selected ==1 && mi.DataPrep.Z_effect(1)==1
    mi.DataPrep.Z_effect(:)=1;
elseif Selected ==1 && mi.DataPrep.Z_effect(1)==0
    mi.DataPrep.Z_effect(:)=0;
end
...
% Hint: get(hObject,'Value') returns toggle state of check_zeroeffect
