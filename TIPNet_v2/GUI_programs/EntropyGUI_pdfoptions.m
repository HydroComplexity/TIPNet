...
pdf = compute_pdfGUI(netdata{1}.data_process2(:,1),mi.N,mi.bin_scheme, mi.Range(:,1),mi.method);
...
pdf = compute_pdfGUI(netdata{1}.data_process2(:,2),mi.N,mi.bin_scheme, mi.Range(:,2),mi.method);
info = compute_info_measures(pdf);
...
if mi.nvars>2
pdf = compute_pdfGUI(netdata{1}.data_process2(:,3),mi.N,mi.bin_scheme, mi.Range(:,3),mi.method);
info = compute_info_measures(pdf);
...
if nodes(2)==0 %compute and plot 1D pdf (line plot)
    ...
    r = mi.Range(:,nodes(1));
    pdf = compute_pdfGUI(dat,N,mi.bin_scheme, r,method);
    axes(handles.plot_pdf)
    bar(1/N:1/N:1,pdf)
    xlim([-.01 1.1])
...
elseif nodes(3)==0 %compute and plot 2D pdf (surface plot)
    ...
    pdf = compute_pdfGUI(dat,mi.N,mi.bin_scheme, r,method);
    axes(handles.plot_pdf)
