...
mi.DataPrep.FilterLambda = ones(1,nvars+1).*mi.nSteps./50;
...
%set h (smoothing parameters) for KDE method, initially for 1D, 2D, 3D pdfs for each node
%based on Silverman (1986) book Ch. 4, Epanichov kernel
for i=1:nvars
  
  dat = mi.DataPrep.dp{i}.data_process2;
  R = range(dat);
  %varterm: variance of non-zero values
  npts = sum(dat~=0); %number non-zero data pts
  
  mi.KDEparams.h1(i)= 1.06 * npts^(-1/5) .* var(dat)./R; 
  mi.KDEparams.h2(i)= 1.77 * npts^(-1/6) .* var(dat)./R; 
  mi.KDEparams.h3(i) = 2.78 * npts^(-1/7) .* var(dat)./R; 
end
